...
| `11_combat_feel.md` | 战斗手感：技能武器绑定、技能后摇、补给滑行（用户反馈 09-11） | 无 | 排队 |
| `12_weapon_facings.md` | 武器方位：侧舷/尾炮接敌、非正面武器自主目标、挂点（用户反馈 09-11） | 07 | 排队 |
| `13_fleet_and_reinforcement.md` | 上阵 7 人、阵营增援、自定义编队存档（用户反馈 09-11） | 无 | 排队 |
| `14_dialogue_face_system.md` | face.tbl 编号体系、每句头像、角色覆盖、面板子目录（用户反馈 09-11） | 无 | 排队 |
| `15_menu_mosaic.md` | 主界面头像俄罗斯方块背景（用户反馈 09-11） | 无 | 排队 |

计划 10（资源目录）已关闭（2026-09-11）：用户确认 `dump/` 是临时目录且
已 git 忽略，无需处理；`assets/` 保留为运行时素材目录。

用户指定的"药引"优先级（2026-09-11）：13 编队增援与 15 主界面背景先做，
14 编号体系随后（编辑器部分可延续）。
