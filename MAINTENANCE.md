...
队伍配置集中在 `teams/`。每个目录就是一支队伍：`teams/default` 是混池角色，`teams/moon` 和 `teams/rune` 是当前两队。队伍目录只放指挥官、左右手、队员、头像、立绘/必杀立绘、左右手半身立绘和战斗 BGM；标题菜单音乐、胜利/失败音乐、通用音效、机体逻辑和 UI 仍使用全局资源。队内 pilot_id 可以在不同队伍重复。新增队伍只需添加 `teams/<id>/team.tbl` 和对应角色目录。
